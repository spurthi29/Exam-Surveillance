...
def run(
        weights=ROOT / 'best11.pt', # model path or triton URL
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        with dt[2]:
            ...
            # pred = utils.general.apply_classifier(pred, classifier_model, im, im0s) # Process predictions
    ...
    # im.txt
     ...
     gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh imc = im0.copy() if save_crop else im0 # for save_crop
            ...
            for c in det[:, 5].unique():
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        # normalized xywh
                        line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format with open(f'{txt_path}.txt', 'a') as f:
                        f.write(('%g ' * len(line)).rstrip() % line + '\n')
                    if save_img or save_crop or view_img:  # Add bbox to image c = int(cls) #integer class
                        label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                        annotator.box_label(xyxy, label, color=colors(c, True))
                        speech = gTTS(names[c], lang='en', slow=False)  # Save the speech as an mp3 file
                                ...
                                if isinstance(vid_writer[i], cv2.VideoWriter): vid_writer[i].release()
                                    ...
                                    if update:
                                        ...
                                        def parse_opt():
                                            ...
                                            parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'best11.pt', help='model path or triton URL')
                                            parser.add_argument('--source', type=str, default='a.mp4', help='file/dir/URL/glob/screen/0(webcam)')
